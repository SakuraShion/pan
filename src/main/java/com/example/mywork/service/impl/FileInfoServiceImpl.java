...
import com.example.mywork.entity.enums.*;
...
import com.example.mywork.utils.ProcessUtils;
...
public class FileInfoServiceImpl implements FileInfoService {
        ...
        try {
            ...
            if (chunkIndex==0){
                ...
                if (!dbFileList.isEmpty()) {
                    ...
                    dbFile.setCreateTime(curDate.toInstant().atZone(ZoneId.systemDefault()).toLocalDateTime());
                    dbFile.setLastUpdateTime(curDate.toInstant().atZone(ZoneId.systemDefault()).toLocalDateTime());
            ...
            //保存临时大小
            redisComponent.saveFileTempSize(webUserDto.getUserId(), fileId, currentTempSize);
            //不是最后一个分片，直接返回
            ...
            fileInfo.setCreateTime(curDate.toInstant().atZone(ZoneId.systemDefault()).toLocalDateTime());
            fileInfo.setLastUpdateTime(curDate.toInstant().atZone(ZoneId.systemDefault()).toLocalDateTime());
    ...
    public FileInfo rename(String fileId, String userId, String fileName) {
        ...
        dbInfo.setLastUpdateTime(curDate.toInstant().atZone(ZoneId.systemDefault()).toLocalDateTime());
    ...
    public void removeFile2RecycleBatch(String userId, String fileIds) {
        ...
        FileInfo fileInfo = new FileInfo();
        fileInfo.setRecoveryTime(new Date().toInstant().atZone(ZoneId.systemDefault()).toLocalDateTime());
    ...
    public void recoverFileBatch(String userId, String fileIds) {
        ...
        fileInfo.setLastUpdateTime(new Date().toInstant().atZone(ZoneId.systemDefault()).toLocalDateTime());
    ...
    public void saveShare(String shareRootFilePid, String shareFileIds, String myFolderId, String shareUserId, String cureentUserId) {
        ...
        for (FileInfo item:shareFileList){
            ...
            if (haveFile!=null){
                item.setFileName(StringUtils.rename(haveFile.getFileName()));
            }
    ...
    public static void union(String dirPath, String toFilePath, String fileName, boolean delSource) throws BusinessException {
        ...
        File targeFile=new File(toFilePath);
        RandomAccessFile writerFile=null;
        try {
            writerFile=new RandomAccessFile(targeFile, "rw");
    ...
    private void updateUserSpace(SessionWebUserDto webUserDto, Long totalSize) {
        ...
        UserSpaceDto spaceDto=redisComponent.getUserSpaceUse(webUserDto.getUserId());
        spaceDto.setUserSpace(spaceDto.getUserSpace()+totalSize);
    ...
    private void findAllSubFolderFileIdList(List<String> fileIdList, String userId, String fileId, Integer delFlag) {
        ...
        List<FileInfo> fileInfoList = this.fileInfoMapper.selectList(query);
        for (FileInfo item : fileInfoList) {
            findAllSubFolderFileIdList(fileIdList, item.getUserId(), item.getFileId(), delFlag);
        }
    }
}
